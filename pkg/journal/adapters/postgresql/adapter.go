...
func NewAdapter(dsn string) (*Adapter, error) {
	...
	log.Infof("connect to PostgreSQL %s", dsn)

	connect, err := databasesql.Open("pgx", dsn)
	if err != nil {
		log.Errorf("unable to open PostgreSQL err: %v", err)
		return nil, err
	}

	if err := connect.Ping(); err != nil {
		log.Errorf("unable to ping PostgreSQL. err: %v", err)
		return nil, err
	}
...
// Insert inserts journal entry into PostgreSQL
...
// FindAll finds entries in PostgreSQL
func (j *Adapter) FindAll(entry *journal.Entry) ([]*journal.Entry, error) {
	e := NewAdapterEntryPostgreSQLSearch().Import(entry)
